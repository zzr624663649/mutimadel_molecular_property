...
class K_BERT_WCL(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # return logits_global, logits_atom, h_global, h_atom
        output = self.embedding(input_ids)
        enc_self_attn_mask = get_attn_pad_mask(input_ids)
        for layer in self.layers:
            output = layer(output, enc_self_attn_mask)
        ...
        h_atom = self.fc(h_atom)
        h_all= self.fc(output)
        h_atom_emb = h_atom.reshape([len(output) * (self.maxlen - 1), self.d_model])
        # logits_atom = self.classifier_atom(h_atom_emb)
        h_global = self.fc(h_global)
        # logits_global = self.classifier_global(h_global)
        return h_all, h_global, h_atom
...
def rdkit_functional_group_label_features_generator(smiles) -> np.ndarray:
    """
...
def run_a_pretrain_epoch(args, args_g, epoch, model_bert, model_grover, model_coca, data_loader,
                         optimizer_bert, optimizer_coca, optimizer_grover, loss_criterion_atom):
    model_bert.train()
    total_loss = 0
    for batch_id, batch_data in enumerate(data_loader):
        ...
        # model_coca.requires_grad_(False)
        ...
        fg_feature = rdkit_functional_group_label_features_generator(smiles)
        fg_feature = torch.tensor(fg_feature)
        #grover
        ...
        fg_feature = fg_feature.float().to(args['device'])
        ...
        text_all, text_global, text_atom = model_bert(token_idx)
        ...
        loss = model_coca(graph_atom, graph_global, text_global, text_atom, atom_labels, fg_feature, text_all, atom_mask, loss_criterion_atom, return_loss=True)
        loss.backward()
        optimizer_coca.step()
        optimizer_bert.step()
        ...
        del token_idx, global_labels, atom_labels, atom_mask, loss, text_all, text_global, text_atom, batchgraph, node_rep
